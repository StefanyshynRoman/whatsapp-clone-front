...
import {createPaginationOption, State} from '../../shared/model/state.model';
...
export class UserSearchService {
  ...
  private listenToSearch(): void {
    this.searchQuery$.pipe(
      ...
      debounce(() => timer(300)),
      switchMap(query => this.fetchResults(query).pipe(
        catchError(err => {
  ...
  private fetchResults(searchQuery: SearchQuery): Observable<Array<BaseUser>> {
